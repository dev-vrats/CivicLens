...
function buildWhatsAppUrl(r: ExtendedPin): string {
  const mapsUrl = `https://maps.google.com/?q=${r.lat},${r.lng}`;
  const date = r.createdAt
    ? r.createdAt.toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })
    : "Unknown date";
  const text = [
    `*CivicLens Report — ${date}*`,
    ``,
    `*Issue:* ${r.description}`,
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)" }}>
          <p style={{ fontSize: 11, color: "var(--text-3)", marginBottom: 8, fontWeight: 500 }}>Message preview</p>
          <div style={{
            background: "var(--bg-3)", borderRadius: 8, padding: "10px 12px",
            fontSize: 12, color: "var(--text-2)", lineHeight: 1.6, fontFamily: "monospace",
            whiteSpace: "pre-wrap", maxHeight: 120, overflowY: "auto",
          }}>
            {[
              `CivicLens Report`,
              `Issue: ${report.description.slice(0, 60)}${report.description.length > 60 ? "…" : ""}`,
              `Location: ${report.lat.toFixed(5)}, ${report.lng.toFixed(5)}`,
              `Map: maps.google.com/?q=${report.lat},${report.lng}`,
              `Photo: [image link included]`,
            ].join("\n")}
          </div>
        </div>
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          <p style={{ fontSize: 11, color: "var(--text-3)", textAlign: "center", marginTop: 10, lineHeight: 1.5 }}>
            Opens WhatsApp with a pre-filled message containing the full report details, map link, and photo. Send it to your local PWD number.
          </p>
        </div>
      </motion.div>
    </motion.div>
